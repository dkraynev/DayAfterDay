-- Rolling daily report: per-user aggregates for Aviator wins with odd >= threshold in user windows
...
SELECT
  ...
  h.FTD_DepositLC,
  COUNT(*) AS BetsCount,
  ROUND(AVG(h.odd), 3) AS AvgOdd,
  COALESCE(t.TotalStakeLC_Window, 0) AS TotalStakeLC_Window
FROM hits h
LEFT JOIN totals t
  ON t.Brand = h.Brand AND t.UserId = h.UserId
GROUP BY h.Brand, h.UserId, h.FD_Date, h.FTD_DepositLC, t.TotalStakeLC_Window
ORDER BY h.UserId;
